...
- 已有只读 GUI 起点：PySide6 仍是 optional `gui` extra，本地开发 `.venv` 可安装 `.[gui]`；当前 `MainWindow` 只展示 workflow / execution / queue / resources-runtime 四个 dockable 区域和 Queue snapshot，不执行任务。
- 尚未实现真实编辑型 GUI、真实 Scheduler、真实 FFmpeg/model adapters、SQLAlchemy repository、打包发布链或插件加载链。

下一步适合继续推进 GUI 只读面板的数据丰富度、workspace layout persistence，或开始拆真实 worker/adapters 计划。
