...
- [docs/plan/plan_main_interactive_gui_workbench.md](./docs/plan/plan_main_interactive_gui_workbench.md)：交互式 GUI 工作台主计划，拆分 Workflow Canvas、Execution Canvas、Queue Monitor、Inspector、workspace 和 motion 后续方向。
- [docs/plan/plan_atelier_ui_local_library_governance.md](./docs/plan/plan_atelier_ui_local_library_governance.md)：`AtelierUI` 本地专属库治理计划，规定自绘控件、动效和共享 GUI 基础的审查与参考优先边界。
- [docs/plan/plan_atelier_ui_foundation.md](./docs/plan/plan_atelier_ui_foundation.md)：`AtelierUI` 最小地基计划，建立本地库目录、theme tokens 和自绘控件入库 checklist。
- [docs/plan/plan_atelier_ui_component_workbench.md](./docs/plan/plan_atelier_ui_component_workbench.md)：`AtelierUI` 控件画板计划，用于候选自绘控件入库前的参数调试、状态展示和用户审查。
- [docs/plan/plan_atelier_ui_component_workbench_foundation.md](./docs/plan/plan_atelier_ui_component_workbench_foundation.md)：`AtelierUI` 控件画板第一实现阶段计划，建立 dev-only PySide6 启动入口、catalog/state、token preview 和 intake checklist preview。
- [docs/plan/plan_atelier_ui_component_workbench_controls.md](./docs/plan/plan_atelier_ui_component_workbench_controls.md)：`AtelierUI` 控件画板第二实现阶段计划，建立 catalog selection、story states / controls metadata 和 controls panel。
- [docs/plan/plan_atelier_ui_component_workbench_screenshot.md](./docs/plan/plan_atelier_ui_component_workbench_screenshot.md)：`AtelierUI` 控件画板第三实现阶段计划，建立手动截图和 JSON 审查备注留痕。
- [docs/plan/plan_atelier_ui_component_workbench_review_page.md](./docs/plan/plan_atelier_ui_component_workbench_review_page.md)：`AtelierUI` 控件画板静态 HTML review page 计划，并明确画板源码应进 GitHub、生成审查产物不进 GitHub。
...
- [atelier/gui/ui/README.md](./atelier/gui/ui/README.md)：`AtelierUI` 本地专属 UI 基础库说明，当前覆盖 theme tokens、自绘控件入库 checklist 和 dev-only component workbench。
