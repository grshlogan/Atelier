...
- `docs/plan/plan_atelier_ui_local_library_governance.md` 已规定：`AtelierUI` 是本地专属库，新自绘控件入库并被软件调用前必须经过用户审查，有参考项目时先调研借鉴。
...
- 后续可拆 `plan_gui_motion_foundation.md`，用于 `easing.py`、`motion.py` 和 reduced motion。
- 后续可拆 `plan_atelier_ui_self_painted_widget_intake.md`，用于某个具体自绘控件进入专属库前的审查。
