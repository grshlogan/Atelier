...
> 本计划推进控件画板的静态 HTML review page。目标是让 PySide6 原生控件画板保存的 PNG / JSON 审查记录，可以生成可在 Codex 浏览器中打开和批注的页面。
...
- 当前画板保存 review snapshot 时也会生成静态 HTML review page。
- 当前画板保存 review snapshot 后会显示最近一次 HTML review page 文件名，供 Codex 浏览器打开批注。
...
- review snapshot 提供相对 screenshot filename、metadata filename、review page filename 和 display fields。
...
- HTML 包含 screenshot、metadata filename、story、states、controls、note。
...
- Red first：`.venv\Scripts\python -m unittest tests.test_gui_atelier_ui_component_workbench` 先因 `review_page_filename`、`render_review_page_html()` 和 `ReviewSnapshotResult.review_page_path` 缺失而失败。
- Green after implementation：`.venv\Scripts\python -m unittest tests.test_gui_atelier_ui_component_workbench` 已通过，9 tests passed。
- Browser handoff red first：`.venv\Scripts\python -m unittest tests.test_gui_atelier_ui_component_workbench` 先因缺少 `component-workbench-review-page-path` label 而失败。
- Browser handoff green：`.venv\Scripts\python -m unittest tests.test_gui_atelier_ui_component_workbench` 已通过，9 tests passed。
- Metadata filename red first：`.venv\Scripts\python -m unittest tests.test_gui_atelier_ui_component_workbench` 先因 `metadata_filename` 缺失而失败。
- Final target：`.venv\Scripts\python -m unittest tests.test_gui_atelier_ui_component_workbench` 已通过，9 tests passed。
- GUI group：`.venv\Scripts\python -m unittest tests.test_gui_atelier_ui_component_workbench tests.test_gui_atelier_ui_foundation tests.test_gui_smoke` 已通过，19 tests passed。
- Full unittest：`.venv\Scripts\python -m unittest discover -s tests` 已通过，141 tests passed。
...
- 2026-05-07：完成 Phase B-C。Review snapshot metadata 新增 `review_page_filename`；新增静态 HTML renderer；保存 review snapshot 时同步写出 `.html`。
- 2026-05-07：完成 Phase D。画板保存 review snapshot 后会显示最近一次 HTML review page 文件名，方便复制给 Codex 浏览器打开批注。
- 2026-05-07：完成 Phase E。接手文档记录 review page 能力和 GitHub 边界；review snapshot / HTML 补齐 `metadata_filename`，避免页面只知道截图和自身文件名。
