...
> 执行顺序：在 `resource_locks + failure recovery` 和只读 PySide6 工作台壳完成后推进。目标是先把 Worker 事件边界稳定下来，再进入真实 subprocess runner。
...
- 已完成首轮状态对齐。`docs/WORKER_PROTOCOL.md` 已从“规划中，尚未实现”改为“部分实现”，并明确 runner、stdin control、heartbeat timeout、stderr 落盘和真实 adapters 尚未实现。
