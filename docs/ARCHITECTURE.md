...
文档契约确认后，下一步进入首版 skeleton：

```text
1. 创建 Python package / pyproject / test baseline
2. 先实现 domain schema、worker event schema、SQLite schema 初始化
3. 加入 RuntimeManager 的 manifest / health check 空实现
4. 接 simulated Worker，跑通 queue -> events -> storage 的最小闭环
