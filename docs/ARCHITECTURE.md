...
首版 skeleton 已开始落地，下一步继续补 resource locks / recovery 或只读 UI 状态面板：
...
4. 已接 sample WorkflowGraph -> ExecutionPlan -> simulated Worker -> SQLite events/artifacts 的最小闭环
5. 已接最小 queue / Scheduler claim
6. 下一步接 resource locks / recovery action，或 PySide6 主窗口和只读状态面板
```
